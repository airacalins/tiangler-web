// Application
export const APP_NAME = "Tiangler";
export const BASE_URL = "https://localhost:5000/api/"

// Strings
export const ACTIONS = "Actions";
export const ADD_PRODUCT = "Add Product";
export const ALREADY_HAVE_AN_ACCOUNT = "Already have an account?";
export const BEARER = "Bearer";
export const CASH = "Cash";
export const CONFIRM_PASSWORD = "Confirm Password";
export const COST = "Cost";
export const CREDIT = "Credit";
export const DASHBOARD = "Dashboard";
export const DONT_HAVE_AN_ACCOUNT = "Don't have an account?";
export const EMAIL_ADDRESS = "Email Address";
export const EMPTY_STRING = "";
export const EXPENSES = "Expenses";
export const FIRST_NAME = "First Name";
export const GROUP = "Group";
export const ID = "id";
export const INVENTORY = "Inventory";
export const IMAGE = "Image";
export const LAST_NAME = "Last Name";
export const LEDGER = "Ledger";
export const LOGIN = "Login";
export const LOGIN_YOUR_ACCOUNT = "Login your account";
export const LOW_STOCK_LEVEL = "Low Stock Level";
export const NAME = "Name";
export const ORDERS = "Order";
export const PASSWORD = "Password";
export const PAYMENT = "Payment";
export const POS = "POS";
export const PRICE = "Price";
export const PRODUCTS = "Products";
export const PURCHASES = "Purchases";
export const REGISTER = "Register";
export const REGISTER_YOUR_ACCOUNT = "Register your account";
export const SERVER_ERROR = "Server Error";
export const SIGN_UP = "Sign Up";
export const STOCKS = "Stocks";
export const SUBMIT = "Submit";
export const TOKEN = "token";
export const UNAUTHORIZED = "Unauthorized";

// Navigation Constants
export const ACCOUNTS_PATH = "/accounts";
export const CASH_PATH = "/cashLedger";
export const CREATE_PATH = "/create"
export const CREDIT_PATH = "/creditLedger";
export const CURRENT_USER_PATH = "/currentUser"
export const HOME_PATH = "/"
export const EXPENSE_PATH = "/expense";
export const INVENTORY_PATH = "/inventory";
export const LEDGER_PATH = "/ledger";
export const LOGIN_PATH = "/login";
export const ORDER_PATH = "/order";
export const PAYMENT_PATH = "/paymentLedger";
export const POS_PATH = "/pos";
export const PRODUCT_PATH = "/product";
export const PURCHASE_PATH = "/purchase";
export const REGISTER_PATH = "/register";


// Form Input Type
export const EMAIL_INPUT_TYPE = "email";
export const NUMBER_INPUT_TYPE = "number";
export const PASSWORD_INPUT_TYPE = "password";
export const TEXT_INPUT_TYPE = "text";


// Error Message
export const MANDATORY_FIELDS = "Please fill up mandatory fields"